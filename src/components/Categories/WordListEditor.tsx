    ...
    >
        <button
            ...
            className={`font-semibold hover:opacity-70 transition-opacity ${
                props.word.negated ? "text-red-700" : "text-green-700"
            }`}
            title={props.word.negated ? "Click to change to AND" : "Click to change to NOT"}
        >
            {props.word.negated ? "NOT" : "AND"}
        </button>
        <span>{props.word.text}</span>
        <button
            onClick={props.onClick1}
            className={`hover:opacity-70 rounded-full p-0.5 transition-opacity ${
                props.word.negated ? "hover:bg-red-200" : "hover:bg-green-200"
            }`}
            title="Remove word"
        >
            <svg
...
function WordListEditor({pattern, onChange}: Readonly<WordListEditorProps>) {
            ...
            {words.length === 0 ? (
                ...
                <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-2">
                        {words.map(word => (
                            <WordLabel key={word.text} word={word} onClick={() => handleToggleNegation(word.text)}
                                       onClick1={() => handleRemoveWord(word.text)}/>
                        ))}
                    </div>
                </div>
            )}
